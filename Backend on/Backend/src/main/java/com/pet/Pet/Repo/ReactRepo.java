...
public interface ReactRepo extends JpaRepository<React,Long> {
    ...
    @Query("SELECT r.reactType FROM React r WHERE r.postId = :postId AND r.postType = :postType AND r.user.id = :userId AND r.isSaved = false")
    Integer findReactTypeByPostIdAndPostTypeAndUserIdAndIsSavedFalse(@Param("postId") Long postId, @Param("postType") int postType, @Param("userId") Long userId);
